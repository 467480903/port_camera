...
def display_combined_frames():
    ...
    while True:
        ...
        if frame1_copy is not None and frame2_copy is not None and cup1 is not None and cup2 is not None:
            ...
            # Calculate vertical difference between cups (y position diff)
            # We'll use the top (y_min) to align them
            cup1_y_top = cup1[1]
            cup2_y_top = cup2[1]
            y_diff = cup2_y_top - cup1_y_top  # Positive if cup2 is lower, negative if cup2 is higher
            ...
            right_part = np.roll(right_part, y_diff*(-1), axis=0)
        ...
        #     break
        # combined_frame_ = np.copy(combined_frame)
...
def main():
    ...
    rtsp_url1 = "rtsp://localhost:8554/cam2"
    rtsp_url2 = "rtsp://localhost:8554/cam3"

    cap1 = cv2.VideoCapture(rtsp_url1)
    cap2 = cv2.VideoCapture(rtsp_url2)
    cap1.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Minimize buffer
    cap2.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Minimize buffer
